    ...
    class EventHubObserver : IObserver<Sensor>
    {
        ...
        public void OnNext(Sensor sensorData)
        {
            try
            {
                ...
                EventData data = new EventData(Encoding.UTF8.GetBytes(serialisedString)) { PartitionKey = sensorData.CountryId.ToString() };
                _eventHubClient.Send(data);
               
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine("Sending" + serialisedString + " at: " + sensorData.EventTime);
